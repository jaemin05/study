...
- [`Mutable`](source/mutable.d.ts) - Convert an object with `readonly` keys into a mutable object. The inverse
  of `Readonly<T>`.
...
- [`RequireExactlyOne`](source/require-one.d.ts) - Create a type that requires exactly a single key of the given keys
  and disallows more.
...
- [`Dictionary`](https://github.com/sindresorhus/type-fest/issues/33) - You only save a few
  ...
  , which is more flexible and well-known. Also, you shouldn't use an object as a dictionary. We have `Map` in
  JavaScript now.
